...
%%TODO dla tzw. kompeltności przydałby się którki 1-3 strony opis układów FPGA - zasobów itp.
...
Oprócz wymienionych wyżej, każdy układ \textit{FPGA} posiada również wiele innych zasobów, do najważniejszych z pewnością należy zaliczyć (szczegółowy opis można znaleźć w dokumentacji technicznej producenta \cite{xilinx_7series_overview}):
...
%%TODO desing to nie polskie słowo !

%%TODO Tu proszę napisać jak było tzn. że chyba w oparciu o istniejące modele powstałe w Lab Bio i z jaką PAna ingerencją.
...
Wykonane implementacje oczywiście nie są w stanie zapewnić przetwarzania obrazu w czasie rzeczywistym. %%TODO podać platformę obliczeniową !
...
Wszystkie metody uruchomiono i sprawdzono na karcie \textit{VC707} z układem \textit{Virtex-7 (\small{XC7VX485T--2FFG1761})}. Platformy z rodziny \textit{Virtex-7} należą do najwydajniejszych układów firmy \textit{Xilinx}. %%TODO złe słowo, proszę sprawdzić jak 485 T ma się do rodziny V7 oraz w którym roku zostały wprowadzone.
...
%%TODO chybna nie wielkość tylko przepustowość do pamięci RAM 
...
%%TODO doprecyzowałbym to. Że to jest taki szablon modułu segmentacji obiektów pierwszoplanowych...
...
W przypadku niektórych algorytmów został on rozszerzony o dodatkowe wejścia/wyjścia. 
	...
	\begin{figure}[h!]
			...
			\caption{Szablon interfejsu wizyjnego} %%TODO zmiana podpisu
...
Istotną kwestią jest również użycie zasobów oraz pobór energii przez układ \textit{FPGA}. %%TODO użycie, bo jakby się zastanowić to zasoby FPGA się nie żużywają...
...
%%TODO tutaj bym jednak prosił o bardziej precyzyjne dane pt. 
%%TODO zasoby "gołego" toru wizyjnego
%%TODO zasoby kontrolera RAM
%%TODO zasoby tego modułu segmentracji
%%TODO suma, żeby się zgadzała z tym w tabeli 4.1

%%TODO poza tym, to jakoś trzeba uporządkować, bo teraz to jest taka wrzutka z tym modułem. MOże właśnie rozbić to na opis toru wizyjengo (może screen z Vivado), potem konteolera RAM, potem tej prostej segmentacji. BTW. nie ma różnicy w zasobach zależnie od rodzielczości obrazu dla kontrolera ?
...
%%TODO raczej mogą być - powinien to Pan rozwinąć.
...
Poprzez pojedynczy blok \textbf{D} rozumiemy opóźnienie o jeden takt zegara, z kolei stała \textit{H\_SIZE} definiuje liczbę pikseli w jednej linii obrazu.
...
Podczas ustalania parametru \textit{H\_SIZE} należy zwrócić uwagę na tzw. obszar synchronizacji. 
...
Wykorzystano go także w wielu innych badaniach przeprowadzanych w Laboratorium Biocybernetyki AGH, między innymi \cite{kryjak_14_vibe, kryjak_14_pbas}. W przeciwieństwie do pozostałych modułów i algorytmów, generator został w całości zaimplementowany w języku \textit{VHDL}. Wykorzystany kod źródłowy został udostępniony przez autorów wymienionych publikacji.
%%TODO napisać, że jest to kod udostępniony przez autorów
...
Cała koncepcja opiera się na wykorzystaniu rejestrów przesuwnych (ang. \textit{shift register}) i modułów \textit{LUT}. %%TODO powt. wykorzystaniu
...
Pamięć \textit{RAM} synchronizowana jest innym zegarem niż sygnał z kamery, w związku z tym konieczne jest zastosowanie buforów w postaci dodatkowych kolejek \textit{FIFO}. %%TODO ze wzglęndu na fakt - średni styl
Omawiany kontroler (moduł \textit{CTRL} na zamieszczonym rysunku) został zaimplementowany jako maszyna stanów. %%TODO napisać, że to moduł CTRL
...
%%TODO może coś o długościach kolejek FIFO...

\section{Implementacja algorytmu ViBE}
\label{sec:fpga_vibe}
%%TODO jasno napisać co Pana, co wcześniej było, czy korzystał Pan z tego co było itp..

Przygotowana implementacja sprzętowa powstała na podstawie opisu teoretycznego metody -- przedstawionego w rozdziale \ref{sec:vibe_teoria}. Wykorzystano wersję operującą w przestrzeni \textit{CIELab}. Autorzy publikacji \cite{kryjak_14_vibe} udostępnili kod źródłowy w języku \textit{VHDL}, jednak przedstawiona w niniejszym rozdziale wersja została zaimplementowana od zera, przy użyciu języka \textit{Verilog}.  
...
Ostatnim etapem jest blok \textit{classification} dokonujący ostatecznej klasyfikacji piksela zgodnie z równaniem (\ref{equ:vibe_match_test}). Dodatkowo, w celu eliminacji szumów, maska wyjściowa jest filtrowana, wykorzystano filtr medianowy o rozmiarze $5x5$. %%TODO raczej maska jest filtrowana
...
Oszacowany w obu przypadkach pobór mocy wynosi \num{3.957}W i \num{3.370}W. %%TODO dla wyższej mnie ? coś to nie tak.... - jest wyjaśnienie na koncu akapitu
W celu uproszczenia implementacji, dla rozdzielczości \textit{720p} i \textit{1080p} zastosowano te same rozmiary kolejek i linii opóźniających (zmieniono jedynie stałą \textit{H\_SIZE}), w związku z tym zarówno pobór mocy jak i wykorzystanie zasobów w obu przypadkach jest identyczne. %%TODO a co z 720p ?
...
Na wyraźnie większe użycie \textit{LUT} miało wpływ między innymi zastosowanie konwersji \textit{RGB}--\textit{CIELab}, gdzie konieczne było tablicowanie wartości niektórych funkcji. %%TODO powt zdecydowanie
...
%%TODO może by jednak wyśrodkować te tabelki ... - próbowałem wyśrodkować, moim zdaniem tak jest bardziej czytelnie
	\begin{table}[h!]
		\centering
		\begin{threeparttable}
...
Z uwagi na dodatkowy mechanizm obsługujący ruch kamery, rozszerzona wersja algorytmu operuje jedynie na rozdzielczości \textit{720x576}. %%TODO napisać dlaczego tzn inny szyk tego akapitu. Ze z uwagi na to, że ten kontekst to...
...
Dla obsługiwanej rozdzielczości parametr \textit{H\_SIZE} wynosi 864, natomiast model tła ma rozmiar 460 bitów. Ostatecznie ustawiono szerokość pojedynczego elementu na 512 bitów, natomiast głębokość kolejki wynosi 32768.  
	...
	\begin{figure}[h!]
		...
		\caption{Implementacja modułu wyznaczającego wektor przesunięcia -- źródło \cite{kryjak_14_vibe}} %%TODO źródło !!!
...
Piksel dla którego otrzymano największą wartość współczynnika $R$ w obszarze każdego bloku zostaje zapisany razem z kontekstem $3x3$ w kolejce \textit{FIFO} (w przypadku obrazu o rozdzielczości \textit{720x576} konieczne jest zdefiniowanie kolejki o rozmiarze 396, gdyż tyle jest bloków $32x32$ w jednej ramce). %%TODO najmniejszą, czy największą ?
...
%TODO to powinno być mocniej powiązane z tym rysunkiem 4.11
...
%TODO tu tak samo, jak to się ma do wcześniejszych badań !
...
Implementacja algorytmu \textit{PBAS}, operującego na jednej składowej \textit{RGB} bądź skali szarości. została przedstawiona na rysunku \ref{fig:pbas_1c_diagram}, jak łatwo zauważyć jest bardzo podobna do implementacji metody \textit{ViBE} przedstawionej w rozdziale \ref{sec:fpga_vibe}. 
%TODO coś styl tego zdania do poprawy
...
%%TODO zastanawia mnię dlaczego wartości dla 720p i 1080p są takie same. To jest jakoś uzasadnione, czy tylko wynikiem uproszczeń.
...
%%TODO też by to trzeba lepiej skomentować.
...
%%TODO bardziej konkretnie co użyte, co przepisane, co zmodyfikwoane, przeprotowane itp...
...
Moduł składa się z dwóch części, pierwszą z nich jest fragment logiki programowej (\textit{Labelling Block}), odpowiedzialny za przypisanie kolejnym pikselom odpowiednich etykiet. %%TODO powt. przygotowy
...
%%TODO dziwnie to brzmi -> raczej w duchu, że oszczędność kosztem dokładności.
